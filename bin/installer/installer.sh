...
function find_php {
    # We need to find and check Plesk's PHP binary, if it exists
    # /opt/psa/admin/bin/php for plesk
    phpLocations=( "/opt/psa/admin/bin/php" "/usr/local/bin/prospamfilter_php" )
...
if [ "$BYPASS" == false ]; then
    ...
    if [[ -z "$php_binary" || ! -e "$php_binary" || -d "$php_binary" ]]; then
        echo ""
        echo -e "\033[31mUnable to detect your Plesk's PHP binary. Please specify that path manually (must be at least PHP5 $REQUIRED_VERSION), followed by [ENTER]:\033[0m"
        echo ""
        read PHP_CUSTOM
...
if [ -d "/usr/local/psa" ];
then
    echo "Installing ProSpamFilter for Plesk.."
    paneltype="plesk"
else
    echo "Unable to detect Plesk installation"
    exit 1
fi
...
if [ "$paneltype" == "plesk" ]; then
    # Add check to see if the API endpoint exists with the proper filename
    if [[ -f "/usr/local/psa/admin/htdocs/enterprise/control/Agent.php" && ! -f "/usr/local/psa/admin/htdocs/enterprise/control/agent.php" ]]; then
        echo ""
        echo "**********************************************************************************"
        echo "[!!] Warning: Plesk API endpoint is using a non-standard filename. "
        echo "We can fix this for you by placing a symlink from Agent.php to agent.php."
        echo ""
        read -n1 -p "Do you want us to create this symlink? (y/n) "
        if [[ $REPLY = [yY] ]]; then
            ln -s /usr/local/psa/admin/htdocs/enterprise/control/Agent.php /usr/local/psa/admin/htdocs/enterprise/control/agent.php
        else
            echo ""
            echo ""
            echo "[!!] You chose NOT to create a symlink. This will mean the addon will not function properly."
            echo "If you want to make the symlink yourself you can make use of the following command:"
            echo ""
            echo ln -s /usr/local/psa/admin/htdocs/enterprise/control/Agent.php /usr/local/psa/admin/htdocs/enterprise/control/agent.php
        fi
        echo ""
    fi
fi
