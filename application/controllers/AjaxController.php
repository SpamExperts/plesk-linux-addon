	...
	public function actionAction()
	{
		...
		switch (strtolower($this->_request->getParam('do'))) {
			...
			case "getcollectiondomains":
				...
				$data = $this->_panel->getCollectionDomains(true);

                /**
                 * In some curcumstances the array of domains should be resorted in a special way -
			...
			case "executebulkprotect":
                ...
                if (! $this->_panel->validateOwnership($domainToValidate)) {
                    $data = $manager->createProtectResponse($domain, "Skip: Validation fail", SpamFilter_Hooks::SKIP_OWNER_VALIDATION_FAIL);
                } else {
            ...
            case "toggleprotection":
                ...
                $manager = new SpamFilter_ProtectionManager();
                $data = $manager->toggleProtection($domain, $owner_domain, $type, $owner_user);
                break;
        }
