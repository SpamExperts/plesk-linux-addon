...
class SpamFilter_Core
{
	...
	public static function getPanelType()
	{
        if (file_exists('/usr/local/psa/') || file_exists($_ENV['plesk_dir'])) {
			Zend_Registry::get('logger')->debug("[Core] Panel is determined to be Plesk.");
    ...
    final static public function getUsername()
	{
        ...
        // We need to check if it is Windows first
        // Because in other way function will return username from system
        if(self::isWindows()){               
            if (class_exists('pm_Session', false) && pm_Session::getClient()->isAdmin()) {
                $logger->debug("[Core] Returning username (admin) as pm_Session tells it's an admin in Plesk 12");
                return 'admin';       
            }
                $client = pm_Session::getClient();
                $login = $client->getProperty('login');
                if(!empty($login)){
                    $logger->debug("[Core] Returning username gathered from pm_Client property 'login'");
                    return $login;
                }
            // If we do not find login by fuctions above we try make API request.
            $logger->debug("[Core] Returning username found by Client ID returned by pm_Session (Plesk 12)");

            $pleskApi = new Plesk_Driver;
            $customerInfo = $pleskApi->doRequest(
                array(
                    'customer' => array(
                        'get' => array(
                            'filter'  => array(
            ...
            if (!empty($customerInfo["customer"]["get"]["result"]["data"]["gen_info"]["login"])) {
                return $customerInfo["customer"]["get"]["result"]["data"]["gen_info"]["login"];
            } else {
                return false;
            }
         }
        ...
        /** @see https://trac.spamexperts.com/ticket/19646 */
            ...
            return 'root';
        } elseif (class_exists('pm_Session', false) && pm_Session::getClient()->isAdmin()) {
            $logger->debug("[Core] Returning username (root) as pm_Session tells it's an admin in Plesk 12");

            return 'root';
        } elseif (class_exists('pm_Session', false) && !empty($loginProperty)){
            $logger->debug("[Core] Returning username (" . pm_Session::getClient()->getProperty('login') . ") as pm_Session");

            return pm_Session::getClient()->getProperty('login');
           
        } elseif (class_exists('pm_Session', false) && pm_Session::getClient()->isClient()) {
            $logger->debug("[Core] Returning username found by Client ID returned by pm_Session (Plesk 12)");
