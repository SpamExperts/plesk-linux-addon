...
//    }
    ...
    public function setInitial($data = array())
    {
        ...
        $config->add_domain_loginfail    = (!empty($data['add_domain_loginfail'])) ? $data['add_domain_loginfail'] : '1';
        ...
        $config->enable_for_resellers    = (!empty($data['enable_for_resellers'])) ? $data['enable_for_resellers'] : '1';
        ...
        $brandingWriter = new Zend_Config_Writer_Ini(array(
            'config'   => $branding,
            'filename' => '/etc/prospamfilter/branding.conf'
        ));

        $brandingWriter->write();
