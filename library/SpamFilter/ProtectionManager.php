...
class SpamFilter_ProtectionManager
{
    ...
    public function __construct()
    {
        ...
        $this->panel = new SpamFilter_PanelSupport_Plesk();
    ...
    public function toggleProtection($domain, $ownerDomain, $domainType, $ownerUser)
    {
        ...
        $inFilter = $this->panel->isInFilter($domain);
     ...
     */
    public function unprotect($domain, $ownerDomain, $domainType, $skipOwnershipValidation = false)
    {
     ...
     * @return array
     */
    public function createUnprotectResponse($status, $rawResult)
    {
        $response =  array(
            'status' => $status,
            'rawresult' => $rawResult
    ...
    public function createProtectResponse($domain, $reason, $rawResult = null, $reasonStatus = "error")
    {
    ...
    private function formatToggleProtectionErrorStatusResponse($domain, $newstatus, $hookError, $hookAdditionalInfo = null)
    {
        ...
        switch ($hookError) {
            ...
            case SpamFilter_Hooks::SKIP_PROCESS_OF_ADDON_DOMAINS_DISABLED:
                $message = $this->translator->_("Processing of alias domains and subdomains is disabled in settings");
                break;
            ...
            case SpamFilter_Hooks::SKIP_EXTRA_ALIAS:
                $reason = $this->translator->_(' because aliases and subdomains will be treated as aliases.');
                break;

            case SpamFilter_Hooks::SKIP_ADDED_AS_ALIAS_NOT_DOMAIN:
                $reason = sprintf($this->translator->_(' because alias domains and subdomains are treated as normal domains and "%s" is already added as an alias.'), $domain);
                break;

            case SpamFilter_Hooks::SKIP_ADDED_AS_DOMAIN_NOT_ALIAS:
                $reason = sprintf($this->translator->_(' because alias domains and subdomains are treated as aliases and "%s" is already added as a normal domain.'), $domain);
                break;
    ...
    private function log($msg)
    {
        ...
        $this->logger->debug("Hook[ProtectionToggler] $msg");
    }
}
